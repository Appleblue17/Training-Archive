...
- 每道题目有多个测试点，提交后会立即评测并返回结果。只有正确通过所有测试点的提交才算 AC（Accepted）并计入成绩和总罚时。
- 队伍的最终排名以题目数量为第一优先级，罚时为第二优先级。若通过题目数量相同，则罚时少的队伍排名靠前。
- “罚时机制”是指在比赛中提交错误的次数所产生的时间惩罚。每次提交错误都会增加 20 分钟罚时。总罚时 = 正确提交时间 + 错误提交次数 * 20 分钟。罚时机制的存在会影响队伍的排名，因此在比赛中需要合理安排提交策略，避免不必要的错误提交。
