...
void dfs(int u){
    ...
    for(int i=0;i<g[u].size();i++){
        ...
        dfs(v);
        while(!q[pt[v]].empty()){
            printf("pq insert %lld,%lld\n",q[pt[v]].top().a,q[pt[v]].top().b);
    ...
    while(!q[pt[u]].empty() && (tmp < q[pt[u]].top() || u==1)){
        printf("cur: %lld,%lld. top: %lld,%lld. Merged. ans+=%lld\n",tmp.a,tmp.b,q[pt[u]].top().a,q[pt[u]].top().b,q[pt[u]].top().a*tmp.b);
