...
如果一个段中每种出现过的颜色都恰好出现 [[$4$][[[]{.strut
style="height: 0.6444em;"}[4]{.mord}]{.base}]{.katex-html
aria-hidden="true"}] 次，则称这个段是罕见的。
...
对于第二组数据，只有相邻两种颜色的分界处可能放置隔板，这两个位置可以分别选择放置或不放置，因此答案为
[[$4$][[[]{.strut
style="height: 0.6444em;"}[4]{.mord}]{.base}]{.katex-html
aria-hidden="true"}]。
...
- 第二行包含 [[$n$][[[]{.strut
    ...
    style="height: 0.15em;"}]{.vlist-r}]{.vlist-t
    .vlist-t2}]{.msupsub}]{.mord}]{.base}]{.katex-html
    aria-hidden="true"}]，表示颜色序列。

### Output

对于每组测试数据，输出一行一个整数，表示合法方案数对
[[$998244353$][[[]{.strut
style="height: 0.6444em;"}[998244353]{.mord}]{.base}]{.katex-html
