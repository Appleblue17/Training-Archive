...
void solve(){
    ...
    for(int i=1; i<=n; ++i){
        id[1][i-1+m+n+m]=i-1+m+n+m; cnt[i-1+m+n+m]=min(n-i+1, m);
    }
    for(int i=2; i<=m; ++i){
        id[1][1-i+m+n+m]=1-i+m+n+m; cnt[1-i+m+n+m]=min(n, m-i+1);
    }
    ...
    // exit(0);
    for(int i=1; i<=n; ++i){
        merge(id[0][i+1], id[1][i-1+m+n+m]);
    }
    for(int i=2; i<=n; ++i){
        merge(id[0][i+m], id[1][i-m+m+n+m]);
    }
    for(int i=1; i<=m; ++i){
        merge(id[0][i+1], id[1][1-i+m+n+m]);
    }
    for(int i=1; i<=m; ++i){
        merge(id[0][i+n], id[1][n-i+m+n+m]);
    }
    for(int i=2; i<=n+m; ++i){
    ...
    while(q--){
        ...
        for(auto t:b0[cid]){
            ...
            ans-=T[1][id[1][lx-ly+m+n+m]&1].get(id[1][lx-ly+m+n+m], id[1][rx-ry+m+n+m]);
