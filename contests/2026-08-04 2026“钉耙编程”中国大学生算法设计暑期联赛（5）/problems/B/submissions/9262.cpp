...
void solve(){
    ...
    for(int i=j;i<=m;i++){
        ...
        //p = p*ismn;
        int p = (wsm-sm1)%mo*fpow(wsm,mo-2)%mo;
        p = (1-p+mo)%mo;
        p %= mo;
        int val = ((sm2)%mo)%mo*fpow((wsm-sm1)%mo,mo-2)%mo;
        printf("%lld %lld %lld %lld %lld\n",p,(int)sm2,(int)wsm,(int)sm1,val);
        //printf("%lld/%lld\n",(int)sm2,(int)(wsm-sm1)%mo);
        int i1p = fpow((1-p+mo)%mo,mo-2);
        
        int pp = (p-fpow(p,n+1)+mo)%mo * i1p%mo;
