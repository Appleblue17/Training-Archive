...
void dfs2(int x, int fa, int id, int rt, int d){
    ...
    for(int i=1, op, l, r, x; i<=m; ++i){
        ...
        if(op==1){
            ...
            for(auto [fa, c]:fr[qv[i]]){
                int d=dis(fa, qv[i]);
                if(d>r) continue;
                if(l-d>mxd[fa]) continue;
                // cout<<fa<<' '<<max(0, l-d)<<' '<<r-d<<' '<<i<<' '<<c<<endl;
                if(fa==qv[i]) c=0;
                add(rt[fa], 0, mxd[fa], max(0, l-d), min(mxd[fa], r-d), i, c);
            }
        }
        else{
            read(x);
