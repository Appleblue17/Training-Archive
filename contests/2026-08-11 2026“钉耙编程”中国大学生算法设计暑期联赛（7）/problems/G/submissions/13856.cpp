...
unordered_map<int, multiset<int> > h[20];
void clr(int d, int p, int l, int r){
    T1[d].clr(p, l, r);
    if(l==r) return ;
    int mid=(l+r)>>1;
    T2[d].clr(p<<1, l, mid); T2[d].clr(p<<1|1, mid+1, r);
    T3[d].clr(p<<1, l, mid); T3[d].clr(p<<1|1, mid+1, r);
