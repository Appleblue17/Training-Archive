...
struct sgt2{
    ...
    int fnd(int p, int l, int r, int L, int R){
        //L<=l, mn1<=R
        if(tr[p].mn1>R) return inf-1;
        if(l==r) return l;
        int mid=(l+r)>>1, ret=inf-1;
        if(L<=mid) ret=fnd(p<<1, l, mid, L, R);
        if(ret==inf-1) ret=fnd(p<<1|1, mid+1, r, L, R);
        return ret;
    }
...
struct sgt3{
    ...
    int fnd(int p, int l, int r, int L, int R){
        //L<=mx1, r<=R
        if(tr[p].mx1<L) return 0;
        if(l==r) return l;
        int mid=(l+r)>>1, ret=0;
        if(R>mid) ret=fnd(p<<1|1, mid+1, r, L, R);
        if(ret==0) ret=fnd(p<<1, l, mid, L, R);
        return ret;
    }
    int fnd2(int p, int l, int r, int L, int R){
        if(tr[p].mx1<L) return -1;
        if(r<=R&&tr[p].mx2<L) return 0;
        if(l==r) return l;
        int mid=(l+r)>>1, ret=-1;
...
void add(int d, int p, int l, int r, int L, int R){
    ...
    if(id[d].find(L)==id[d].end()) id[d][L]=++idx;
    if(id[d].find(R)==id[d].end()) id[d][R]=++idx;
...
int get(int d, int p, int l, int r, int L, int R, sgt2::info llim, sgt3::info rlim){
    ...
    if(L<=l&&r<=R){
        ...
        if(r2<l2){
            int ret=0;
            if(max(l, r2+1)<=min(r, l1-1)){
                sgt1::info f=T1[d].get(p, l, r, max(l, r2+1), min(r, l1-1));
                if(f.mn==0) ret+=f.c0;
            }
            if(max(l, r1+1)<=min(r, l2-1)){
