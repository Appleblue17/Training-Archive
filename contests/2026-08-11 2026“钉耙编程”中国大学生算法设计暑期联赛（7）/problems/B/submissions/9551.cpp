...
void solve(){
    ...
    for(int i=1; i<=n; ++i){
        ...
        for(int j=sl[i]; j<=sr[i]; ++j){
            ...
            int q=fnd(p);
            if(p==q) continue;
            for(int k=1; k<lim&&k<len[i]; ++k){
                tmp[k+1]=(tmp[k+1]+D[k].get(dfn2[p]));
            }
            if(q!=0){
                for(int k=1; k<lim&&k<len[i]; ++k){
                    tmp[k+1]=(tmp[k+1]+mod*2-D[k].get(dfn2[q]));
                }
            }
        }
        for(int j=1; j<=len[i]&&j<=lim; ++j){
            ...
            D[j].add(dfn2[p], dfn2[seq[dfn[p]+sz[p]-1]], tmp[j]);
        }
        p=1;
        for(int j=sl[i]; j<=sr[i]; ++j){
            p=A.tr[p][s[j]-'a'];
