...
struct solver{
    ...
    unordered_map<ll, pii> h;
    pii srk(int l, int r){
        if(r<l) return mapa(l, r);
        ll hs=inf*l+r;
        if(h.find(hs)!=h.end()) return h[hs];
        int s[3]={0, 0, 0};
        s[0]=sum[r][0]-sum[l-1][0];
        s[1]=sum[r][1]-sum[l-1][1];
        s[2]=sum[r][2]-sum[l-1][2];
        if(chk(s[0], s[1], s[2])) return h[hs]=mapa(l, r);
        if(s[0]==s[1]&&s[1]==s[2]){
            int nr=jmp3[r];
            if(nr<l){
                return h[hs]=mapa(l, l-1);
            }
            return h[hs]=srk(l, r-3);
        }
        if(s[0]==s[2]&&s[1]>s[0]){
            int op=1, op2=0;
            int cnt=0, nr=r;
            ...
            if(s[op]-cnt>s[op2]){
                return h[hs]=srk(l, nr-1);
            }
            else{
                return h[hs]=srk(l, r-(s[op]-s[op2]));
            }
        }
        if(s[0]==s[1]&&s[2]>s[0]){
            int op=2, op2=0;
        ...
        if(s[0]==s[2]) op=1;
        if(s[0]==s[1]) op=2;
        if(a[r]==op) return h[hs]=srk(l, jmp[r]-1);
        return h[hs]=srk(l, r-1);
    }
    void init(){
        for(int i=1; i<=n; ++i){
