...
有 $n$ 个整数 $a_1,a_2, \ldots ,a_ n$，初始时它们全部等于 $0$。
...
保证将所有$\lparen x_i,y_i \rparen$ 看作无向边后，它们构成一棵包含 $n$ 个点的树。
...
第二行输出 $n-1$ 个整数 $s_1,s_2,\ldots,s_{n-1}$。其中 $s_i$ 必须等于 $x_i$ 或 $y_i$，表示在第 $i$ 次操作中选择 $a_{s_i}$ 加一，并将另一个数清零。
