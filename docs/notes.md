...
- 主功能可用（竞赛列表、文件查看、日志、搜索、Dashboard、复盘时间轴）；爬虫双模式已建（`--contests-only` + `--submissions-only`），由自托管脚本调度（`server-task.sh`：`fire` / `sync` / `incremental`，cron 表达式在 `crawler/config.json` 的 `scheduled` 块）。**定时尚未启用**（未实测通过前手动触发验证）。
...
- 闹钟机制已实现（`crawler/scripts/alarm.py` + `crawler/server-task.sh sync/fire`）：订阅条目可选填 `end_time`，未来比赛写闹钟表 `crawler/alarms.json`（gitignore），到点由 `fire` 爬取 + 立即生成报告；状态模型 `planned` / `pending` / `archived` / `failed`。
...
crawler/server-task.sh incremental  # 提交增量同步（--submissions-only；每日）
crawler/server-task.sh sync         # 同步订阅：历史/过期立即爬，未来写闹钟表（自动 + 可手动）
crawler/server-task.sh fire         # 闹钟到点触发（无到期安静退出）
crawler/server-task.sh install      # 安装 cron（从 config.json 的 scheduled 块读取表达式）
