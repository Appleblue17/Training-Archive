...
- **qq-bot 订阅管理 / 同步**：`/subs add <link> [end_time] [备注]` 写 `crawler/subscriptions/qqbot.json`（bot 管理文件，按约定与其他订阅文件合并加载）；`/subs del <link>` 从所有订阅文件移除匹配 link；两种改动都会后台触发一次 `daemon.py sync` 并在群里回复结果。`/subs` 与 `/sync` 仅 `deploy` 分支工作区生效（`PROD_BRANCH` 保护，防止在非生产分支误改订阅）。`/fortune` 确定性种子 = md5(`scope:user_id:北京日期:salt`)，salt 可配 `config.json` 的 `qq.fortune_salt`（缺省固定值 `training-archive`）。
