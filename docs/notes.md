...
- 主功能可用（竞赛列表、文件查看、日志、搜索、Dashboard、复盘时间轴）；爬虫双模式工作流已建（方式一 Actions：`--contests-only` + `--submissions-only`；方式二服务器：`fire` / `sync` / `incremental`，cron 表达式在 `crawler/config.json` 的 `scheduled` 块）。**定时尚未启用**（未实测通过前手动触发验证）。
...
- **闹钟机制（方式二专用）**：订阅条目可选填 `end_time`（北京时间 ISO）。`server-task.sh sync` 先 `plan` 分类：不填 = 历史比赛立即爬不生成报告；已过 = 立即爬 + 报告；未来 = 写闹钟表；`failed` = 输出 `RETRY` 重试一次（成功 → `archived`，失败保持 `failed`，`plan` 输出 `WARNING`）。`fire`：`due` 无到期闹钟安静退出，有则爬取（`--contests-only --links`）+ 报告 + `mark --archived`；**爬取失败 `mark --failed`**（fire 只查 `planned`，失败后不再自动重试，靠自动 `sync` 兜底）。订阅修改 `end_time` → 重新安排，订阅删除 → 剪除闹钟。方式一（Actions 轮询）不读取 `end_time`。注意：`--links` 与 `--submissions-only` 互斥、无值报错。
