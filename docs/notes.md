...
- 2026-08-12：**cron 配置外置到 `config.json` 的 `scheduled` 块 + 新增每 3 小时自动 `sync`**——`crawler/config.json`（与 `config.example.json`）新增 `scheduled` 块存三条 cron 表达式：`fire`（闹钟检查，**每分钟 → 每 5 分钟**）、`sync`（订阅同步，新增，**每 3 小时**）、`incremental`（提交增量，每日 4:00）。`server-task.sh install` 改为用 `pick_python` 读该块生成 crontab（缺失/非法回落默认值），删除原先硬编码的 `CRON_FIRE`/`CRON_B` 与 UTC 时区自动调整逻辑；`sync` 由「仅手动」变为「cron 每 3 小时自动 + 可手动」，自动兜底 failed 重试（fire 失败后不再需要人工手动 sync）。README / architecture / roadmap / notes 命令速查、当前状态、注意事项同步更新。验证：bash -n / config JSON 解析通过。
- 2026-08-12：**去除「任务A/B」别名，统一以 `scheduled_task.py` 模式指代**——`server-task.sh` 删除 `run [a|b]` 子命令，新增 `incremental`（内部直接调 `scheduled_task.py --submissions-only`，保留 git pull / env / commit 流程），install 的 cron 每日条目改为 `server-task.sh incremental`；`--contests-only` 对应「比赛抓取」路径（方式一 Actions 每 30 分钟、方式二 `sync`/`fire` 内部使用），`--submissions-only` 对应「提交增量同步」（每日），默认模式（无参数）保留为手动/临时。scheduled_task.py docstring、README、architecture、roadmap、notes 命令速查/当前状态/注意事项同步去 A/B 措辞；notes 历史条目保留原表述。验证：bash -n / py_compile 通过。
...
- 主功能（竞赛列表、文件查看、日志页）可用；爬虫双模式工作流已建（方式一 Actions：`--contests-only` 每 30 分钟、`--submissions-only` 每日；方式二服务器：闹钟检查每 5 分钟 + 订阅同步每 3 小时 + 提交增量每日，cron 表达式统一放 `crawler/config.json` 的 `scheduled` 块），**定时尚未启用**（未实测通过前手动触发验证），HDU / NowCoder 爬虫默认停用（由 `crawler/config.json` 的 `enabled` 字段控制，可改回 `true` 启用）。
- 方式二**闹钟机制已实现**（`crawler/scripts/alarm.py` + `crawler/server-task.sh sync/fire`）：订阅条目可选填 `end_time`，未来比赛 `sync` 写闹钟表 `crawler/alarms.json`（gitignore）、cron 每 5 分钟 `fire` 到点爬取 + 立即生成报告；不填 = 历史比赛立即爬不生成报告；已过 = 过期比赛立即爬 + 报告。状态模型：`planned` / `pending` / `archived` / `failed`，`due` 只查 `planned`，failed 由自动 `sync`（每 3 小时）重试一次（成功 → archived，失败保持 failed）。详见「最近更新」与 `docs/architecture.md` §4.6。
...
- **闹钟机制（方式二专用）**：订阅条目可选填 `end_time`（比赛结束时间，北京时间 ISO），由 `crawler/scripts/alarm.py` 读写闹钟表 `crawler/alarms.json`（gitignore，**不提交**，仅服务器本地状态）。状态模型 `planned`（未来等 fire）/ `pending`（sync 待立即处理）/ `archived`（已处理完，fire 忽略）/ `failed`（爬取失败，fire 忽略）。`server-task.sh sync` 先 `plan` 分类：不填 = 历史比赛立即爬取归档不生成报告；已过 = 过期比赛立即爬取 + 报告（闹钟失败后补漏场景）；未来 = 写闹钟表；**failed = 输出 `RETRY` 重试一次**（`plan` 输出 `WARNING` 提示用户）。cron 每 5 分钟 `fire`：`due` 无到期闹钟**安静退出**（不产生提交），有则爬取（`--contests-only --links`）+ 报告（`report.py --from-crawl --links`）+ `mark --archived`；**爬取失败 `mark --failed`**（fire 只查 planned，失败后不再自动重试，下次 `sync` 重试一次：成功 → `archived`，失败保持 `failed`；`sync` 每 3 小时自动跑，相当于低频自动兜底）。`due` 只查 `planned` 且 `fire_at` 已到，pending/archived/failed 一律忽略。订阅修改 `end_time` → 重新安排（archived 也重新激活），订阅删除 → 剪除闹钟。方式一（Actions 轮询）不读取 `end_time`、保持原样。注意：`--links` 与 `--submissions-only` 互斥、无值报错；`--links` 只抓指定订阅，语义与 `--contests-only` 一致（有新建才回填其提交、不推进 last-update）。
...
- **`--contests-only` 模式**（高频触发专用）：只检查订阅有没有触发，有新建比赛才回填其提交（截止 = 该场 `start_time`），无新建完全不碰提交。**不推进 `last-update.json`**——已有比赛的增量由每日 `--submissions-only` 负责；若在 contests-only 推进，会跳过已有比赛在两次任务之间的新提交，造成漏抓。默认模式（`crawler/scripts/scheduled_task.py` 无参数）保留为手动/临时场景。注意：若某场新建比赛的**提交回填失败**（如超时），其文件夹已存在，后续 contests-only 运行不会把它当作"新建"而跳过回填（与默认模式同样的既有限制），需手动处理（重置该平台 last-update 触发全量重抓，或跑默认模式）。
...
python3 crawler/scripts/scheduled_task.py --contests-only     # 只查订阅/新建比赛，有新建才回填其提交（高频触发）
python3 crawler/scripts/scheduled_task.py --contests-only --links "https://qoj.ac/contest/123"  # 只抓指定订阅链接（闹钟 fire/sync 内部使用）
python3 crawler/scripts/scheduled_task.py --submissions-only  # 提交增量模式：每日增量同步提交
...
crawler/server-task.sh incremental  # 提交增量同步（--submissions-only；每日，install 的 cron 自动调用）
crawler/server-task.sh sync        # 同步订阅：历史/过期立即爬，未来写闹钟表（cron 每 3 小时自动 + 可手动）
crawler/server-task.sh fire        # 闹钟到点触发（cron 每 5 分钟；无到期安静退出）
crawler/server-task.sh install     # 安装 cron（从 config.json 的 scheduled 块读取：fire 每 5 分钟 + sync 每 3 小时 + incremental 每日）
