...
- **`commit_and_push` 排除已跟踪的运行时文件**（`daemon.py`）：`git add crawler` 会把历史遗留被跟踪的 `daemon.log` / `daemon-state.json` / `global.log.json` / `platforms/*/log.json` 的改动一起暂存（gitignore 对已跟踪文件无效），导致每次 `[auto] [contests-changed]` 提交都夹带运行时日志噪音。修复：提交前统一 `git reset HEAD` 排除这些文件
